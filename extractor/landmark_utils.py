...
#     face = faces[0]
#
#     landmarks = predictor(frame, face)#<class '_dlib_pybind11.full_object_detection'>
#     #print(type(landmarks.parts()),landmarks.parts())#<class '_dlib_pybind11.points'>
#     face_landmark_list = [(p.x, p.y) for p in landmarks.parts()]#list
#     shape = np.array(face_landmark_list)#np.ndarray
#     #print(shape)
#     return 1, shape

def predict_single_frame(frame, mp_face_mesh, mp_model):
    # mp_face_mesh = mp.solutions.face_mesh
    # mp_model = mp_face_mesh.FaceMesh(
    ...
    # plt.imshow(img_RGB)
    # plt.show()
    results = mp_model.process(img_RGB) # results:<class 'mediapipe.python.solution_base.SolutionOutputs'>
    #dir(results.multi_face_landmarks[0])
    if results.multi_face_landmarks is None: # 防止检测过程中丢失人脸
        return 0,[] 
    else:
        landmarks = results.multi_face_landmarks[0]  
    # print("landmarks", len(landmarks.landmark))
    face_landmark_list = []
    for idx, landmark in enumerate(landmarks.landmark):
        ...
        #     continue
        landmark_px = mp.solutions.drawing_utils._normalized_to_pixel_coordinates(landmark.x, landmark.y,image_cols, image_rows)
        if landmark_px is None:
            face_landmark_list.append(face_landmark_list[-1]) # 超出屏幕外的人脸landmark可能为None，之后比较时会出现问题，取相邻元素填充
        else:
            face_landmark_list.append(landmark_px)
    # print("face_landmark_list", face_landmark_list)
    shape = np.array(face_landmark_list)
    # print(shape.shape)
    face_landmark_list = []
    # for idx,landmark in enumerate(landmarks.landmark):
    #     face_landmark_list.append((landmark.x, landmark.y))
    # shape = np.array(face_landmark_list)
    return 1,shape
...
def detect_frames_track(frames, fps, use_visualization, visualize_path, video):
    ...
    """
    Use single frame to detect face on mediapipe
    """
    # ----------------------------------------------------------------------------#
    mp_face_mesh = mp.solutions.face_mesh
    mp_model = mp_face_mesh.FaceMesh(
    ...
    for i in tqdm(range(frames_num)):
        frame = frames[i]
        face_num, shape = predict_single_frame(frame, mp_face_mesh, mp_model)
        #plt.imshow(shape)
        #plt.show()
        if face_num == 0:
